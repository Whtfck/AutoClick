...
- 新增 `logs` 目录自动写入运行日志，可快速排查异常。
- 为配置引入 `PollIntervalMs`、`NoMatchDelayMs` 参数，默认限制匹配频率至 8~15Hz，减少 CPU 抢占导致的游戏卡顿。
- 抓取与模板匹配环节增加性能跟踪 (`PERF` 日志)，界面启动时附带 CPU/RAM 监控线程，便于分析瓶颈。
- 捕获流程增加 PrintWindow 和桌面复制备选路径，兼容 Win11 下 BitBlt 截图失败的场景。
